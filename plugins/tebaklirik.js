...
`.trim()
    conn.tebaklirik[id] = [
        await conn.sendButton(m.chat, caption, 'HamzzBot By Hamzz', 'Bantuan', `.teli`),
        json, poin,
        setTimeout(async () => {
            if (conn.tebaklirik[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, 'HamzzBot', 'Tebak Lirik', `.tebaklirik`, conn.tebaklirik[id][0])
