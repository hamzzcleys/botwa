...
const Bpaus = 10000
const Spaus = 7000
const Bkepiting = 3000
const Skepiting = 1500
const Bgurita = 8000
const Sgurita = 4000
const Bcumi = 400
const Scumi = 200
...
`.trim()
        ...
        if (/fishop|tokoikan/i.test(command)) {
            const count = args[2] && args[2].length > 0 ? Math.min(99999999999999999999, Math.max(parseInt(args[2]), 1)) : !args[2] || args.length < 4 ? 1 :Math.min(1, count)
            switch (jualbeli) {
            case 'buy':
                switch (_type) {
                    case 'paus':
