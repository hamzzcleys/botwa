...
let handler = async (m, { conn, usedPrefix: _p, args, command }) => {
  ...
  try {
    ...
    if (teks == '404') {
      return conn.relayWAMessage(conn.prepareMessageFromContent(m.chat, {
        "listMessage": {
          ...
          "description": `ketik untuk memilih kategori menu`.trim(),
    ...
    await conn.send2ButtonLoc(m.chat, await (await fetch(`https://telegra.ph/file/df74f7a30610dd38a4be1.jpg`)).buffer(), text.trim(), '©HamzzBot By Hamzz', '👤 OWNER', '.owner', '📢 RULES', '.rules', m)
